...
// Adapted from the 21st.dev "Mesh drift" shader builder output. Stripped of
// TypeScript, pointer/cursor interactivity (not needed for a static auth
// background), and re-tuned to a strictly monochrome palette — see PALETTE
// below. The fragment shader's math is left untouched.
...
// Strictly grayscale (R === G === B on every entry). With neutral inputs the
// shader's hue-rotate/OKLab/saturation math is a mathematical no-op (verified
// against the formulas above), so this is what actually guarantees the
// background can never drift toward a hue — not just a visual approximation.
const GRAY = (v) => [v, v, v]
const PALETTE = {
  colors: [
    GRAY(0x05 / 255), // #050505
    GRAY(0x0a / 255), // #0a0a0a
    GRAY(0x16 / 255), // #161616
    GRAY(0x30 / 255), // #303030
    GRAY(0x50 / 255), // #505050 — brightest tone left; no more near-white
    GRAY(0x50 / 255), // unused filler (colorCount stops at 5)
    GRAY(0x50 / 255), // unused filler
    GRAY(0x50 / 255), // unused filler
  ],
  colorCount: 5,
}
...
const UNIFORMS = {
  ...
  contrast: 1.0,
  brightness: -0.12,
  saturation: 0, // forces grayscale luma mixing (no-op on already-neutral colours, kept for clarity/robustness)
  hue: 0,
  vignette: 0.6,
  blur: 0.03,
  grain: 0, // grain already comes from the existing .auth-noise overlay
  seed: 1453.0,
  rotate: 0,
  offsetX: 0,
  offsetY: 0,
  drift: 0.12,
  oklab: 0,
  timeScale: 0.15, // slow, elegant drift — not the reference's brisk "aurora" pace
}
...
export default function ShaderBackground({ className }) {
  ...
  useEffect(() => {
    ...
    // No pointer interactivity on this screen — offset/mouse and cursor stay
    // at rest for the component's lifetime, so these are set once, not per frame.
